...
```sql
SELECT COUNT(*) AS "송장수" FROM invoices WHERE BillingState is NULL ORDER BY InvoiceDate DESC LIMIT 5;
```
```
송장수
---
...
```sql
SELECT Country AS "나라", COUNT(*) AS "고객 수" FROM customers GROUP BY Country ORDER BY COUNT(*) DESC LIMIT 5;
```
```
C LIMIT 5;
나라       고객 수
...
```sql 
SELECT ArtistId,COUNT(*) AS '앨범 수' FROM albums GROUP BY ArtistId HAVING COUNT(ArtistId) > 10 ORDER BY COUNT(ArtistId) DESC;
```
```
ArtistId  앨범 수
--------  ----
...
```sql 
SELECT COUNT(*) AS "고객 수" ,Country,State FROM customers WHERE State IS NOT NULL GROUP BY Country, State ORDER BY '고객 수', Country DESC Limit 5;
```
```
고객 수  Country  State
----  -------  -----
...
SELECT
CustomerId, 
CASE WHEN Fax IS NULL THEN 'X' WHEN Fax IS NOT NULL THEN 'O' 
END AS 'Fax 유/무'
FROM customers
LIMIT 5;
```
...
```sql 
SELECT Name FROM genres WHERE GenreId = (SELECT GenreId FROM tracks GROUP BY GenreId HAVING COUNT(GenreId) ORDER BY COUNT(GenreId) LIMIT 1);
```
```
Name
-----
