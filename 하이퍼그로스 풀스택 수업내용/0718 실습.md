...
# input().split()으로 입력된 자료형이 str이기 때문에 int로 바꿔준다.
```
...
# 입력받은 내용을 map함수를 통해 int형으로 변환하는 코드에 문자열을 입력해 정수형으로 변환이 불가능하여 오류가 발생한다.
```
...
# len은 숫자에 사용할수 없기때문에 문자열로 변환해준다.
```
...
# answer은 튜플이 아닌 리스트로 선언해준다.
# range의 범위를 1부터 10까지로 수정한다.
```
...
# count+=1 의 indent를 올바르게 수정한다.
# total // count의 몫이 아닌 답을 구한다
```
...
# if문에서 fruit_count = {fruit: 1} 를 통해 fruit_count 딕셔너리를 지속적으로 초기화하고있다. fruit_count[fruit] = 1 를 통해 딕셔너리에 추가하는 코드로 변경해준다.
```
...
# max = max(number_list) 해당 코드에서 변수를 max라는 예약어로 선언하여 max() 내장함수 사용시 오류가 발생하였다. 변수 이름을 수정한다.
```
...
# 함수에서 반환되는 값이 없어서 None를 반환한다. return 값을 추가해준다.
```
